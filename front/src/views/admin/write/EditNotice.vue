...
export default {
  ...
  created() {
    authAPI //통신코드
        .list(this.editArticleId)
        .then((res) => {
          console.log(res.data);
          
          this.articleId = res.data.article.articleId;
          this.title = res.data.article.title;
          this.body = res.data.article.body;
          this.nickname = res.data.article.nickname;
          this.regDate = res.data.article.regDate;
          this.hit = res.data.article.hit;

          this.MYPAGE = res.data.MYPAGE;
          this.comments = res.data.list;
          //this.files = res.data.articleImageFile; //이미지

          for (let i = 0; i < res.data.articleImageFile; i++) {
  ...
  methods: {
    ...
    deleteArticle(){
      if(confirm("삭제하시겠습니까?")){
          authAPI.deleteArticle(this.articleId)
            .then(res=>{
              if(res.data == "ok"){
                alert("삭제되었습니다.")
                return this.$emit('changeToggle', 'change')
